...
import { Stethoscope, Download, Search, Star, Shield, Trash2, UserCheck } from 'lucide-react';
...
export default function DoctorsPage() {
    ...
    const fetchDoctors = async () => {
        ...
        try {
            ...
            const response = await api.get(`/api/admin/doctors?${params.toString()}`);
    ...
    const handleToggleStatus = async (doctor: any) => {
        ...
        try {
            await api.patch(`/api/admin/users/${doctor._id}/status`);
            toast.success(`User ${doctor.isActive ? 'deactivated' : 'activated'} successfully`);
            fetchDoctors();
        } catch (error) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            await api.delete(`/api/admin/users/${selectedDoctor._id}`);
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-secondary-600 mt-2">Manage and view all registered doctors</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <Card padding="lg">
                    <p className="text-sm font-medium text-secondary-500">Total Doctors</p>
                ...
                <Card padding="lg" className="border-l-4 border-green-500">
                    <p className="text-sm font-medium text-secondary-500">Active Doctors</p>
                                ...
                                {doctors.length === 0 ? (
                                    ...
                                    doctors.map((doctor, idx) => (
                                        <tr key={idx} className="border-b border-secondary-100 hover:bg-secondary-50">
                                            <td className="py-3 px-4">
                                                <div className="flex items-center">
                                                    <div className="flex-shrink-0 h-10 w-10 bg-green-100 rounded-full flex items-center justify-center text-green-600 font-bold">
                                                        {doctor.profile?.firstName?.[0] || doctor.email[0].toUpperCase()}
                                                    </div>
                                                    <div className="ml-3">
                                                        <div className="text-sm font-medium text-secondary-900">
                                                            Dr. {doctor.profile?.firstName} {doctor.profile?.lastName}
                                                        </div>
                                                        <div className="text-sm text-secondary-500">{doctor.email}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="py-3 px-4">
                                                <Badge variant={doctor.isEmailVerified ? 'success' : 'warning'}>
                                            ...
                                            <td className="py-3 px-4 text-right">
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`p-1.5 rounded-lg transition-colors ${doctor.isActive === false
                                                            ? 'text-green-600 hover:bg-green-50'
                                                            : 'text-warning-600 hover:bg-warning-50'
                                                            }`}
                                                        title={doctor.isActive === false ? 'Activate' : 'Deactivate'}
            ...
            >
                <div className="space-y-4">
                    <p className="text-secondary-600">
                        Are you sure you want to delete <strong>Dr. {selectedDoctor?.profile?.firstName} {selectedDoctor?.profile?.lastName}</strong>? This action cannot be undone.
                    </p>
                    <div className="flex justify-end gap-3">
                        ...
                        >
                            {actionLoading === selectedDoctor?._id ? 'Deleting...' : 'Delete Doctor'}
                        </Button>
                    </div>
                </div>
            </Modal>
